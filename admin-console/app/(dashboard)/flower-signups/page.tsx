...
import type { FlowerSignup, PledgeType } from '@/lib/types';

const DEMO_MODE = process.env.NEXT_PUBLIC_DEMO_MODE === 'true';

type Row = FlowerSignup;
...
export default function FlowerSignupsPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (fetchError) {
      setError('Failed to load flower signups');
    } else {
      setSignups(
        (data ?? []).map((row: any) => ({
          id: row.id,
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(id);
    const res = await fetch(`/api/flower-signups/${id}`, { method: 'DELETE' });
    if (!res.ok) {
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Flower Signups</h1>
        <p className="text-gray-500 text-sm">Members who have signed up to donate flowers</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-gray-500 text-sm">No flower donation signups yet.</div>
      ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-gray-100 bg-gray-50">
                      ...
                      <th className="text-left px-4 py-3 font-semibold text-gray-600">Signed Up At</th>
