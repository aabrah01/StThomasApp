...
// left the whole of Sunday open, so a member could pledge food at 10:49 for a
...
const FoodDonationCard = React.memo(({ eventDate, eventId, startDate, onSignupChange, refreshKey }) => {
  ...
  const load = useCallback(async () => {
    ...
    const [countResult, signupsResult, fullResult, churchResult] = await Promise.all([
      ...
      databaseService.getMealFullPledge(eventId),
  ...
  const handlePledge = async (pledgeType) => {
    ...
    const { error: err } = await databaseService.createMealSignup(
  ...
  const confirmPledge = () => {
    ...
    if (canPledgeFull) {
      Alert.alert(
        'Pledge To Donate Food?',
        `${longDate(eventDate)}. The church office is notified when you pledge.`,
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: `Share The ${shortDate(eventDate)} Food Donation`,
            onPress: () => handlePledge('shared'),
          },
          {
            text: `Donate Food On ${shortDate(eventDate)} By Myself`,
            ...
            <>
              {churchProvided ? (
                ...
                <View style={styles.notice}>
                  ...
                  <Text style={styles.noticeText}>
                    A member has pledged to donate the food for this service
                  </Text>
                </View>
              ) : !fullPledged ? (
              ...
              {ownSignup ? (
                <View style={styles.ownRow}>
                  <Text style={styles.ownText}>
                    {ownFullPledge
                      ? 'You are donating the food for this service'
                      : ownSignup.pledgeType === 'shared'
                      ? 'You have pledged to share the food'
                      : 'You have pledged to bring food'}
                  </Text>
                  {!isPast && (
                    <TouchableOpacity
